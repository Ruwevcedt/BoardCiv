...
def session_required(
    ...
    def decorated_function(
        ...
        return func(
            *args,
            **kwargs
            ) if 'id' in session else redirect(
            '/'
            )
...
def room(
    ...
    if room_by_host_session_id:
        ...
        room_by_host_session_id = Room.Room(
            host_session_id
            )
        games.append(
            room_by_host_session_id
            )
    else:
        return redirect(
            '/hall'
            )  # todo: 무조건 /로 가버리는디 왜이란지 몰루...
    return render_template(
        'room.html',
        room=room_by_host_session_id
        )
